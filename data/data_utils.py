...
class BalanceNode():
    ...
    def __init__(self, node_num = 4, init_node = None, node_span:list = [0, 200]) -> None:
        self.node_num = node_num
        self.max_node = node_span[1]
        self.min_node = node_span[0]
        assert init_node is None or \
            (init_node[0]==node_span[0] and init_node[-1]==node_span[-1])
        if not init_node:
            init_node = self._init_node()
            if init_node[0] != 0:
                init_node[0] = 0

        self.node = init_node
    def _init_node(self):
        return sorted(random.sample(range(self.min_node, self.max_node + 1), \
            self.node_num))

    def _get_interval(self, node=None):
        """This function is to get the speed interval:
        [0, x1], [x1, x2], ..., [xi, xi+1]"""
        if not node:
            node = self.node

        num_intervals = len(node) - 1
        intervals = np.zeros((num_intervals, 2))
        for i in range(0, num_intervals):
            intervals[i, :] = node[i:i+2]
        
        # self.intervals = intervals
        return intervals

    @staticmethod
    def _get_interval_width(intervals):
        interval_width = [interval[1] - interval[0] for interval in intervals]
        return interval_width
        

class DataStatistic(BalanceNode):
    def __init__(self, dataset_root=None, save_path=None, voxel_size=(0.25, 0.25, 0.4),
                                split='train', area_extents = np.array([[-32., 32.], [-32., 32.], [-3., 2.]]),
                                num_obj_cat=5,
                                num_motion_cat=3,
                                intervals=\
                                np.array([[0.0, 0.0], [0, 5.0], [5.0, 20.0], [20.0, 200]])) -> None:
        self.dataset_root = dataset_root
        self.save_path = save_path
        self.voxel_size, self.area_extents = voxel_size, area_extents
        self.split = split
        self.num_obj_cat = num_obj_cat
        self.num_motion_cat = num_motion_cat
        
        self.speed_intervals = intervals
        self._load_data()
        # super.__init__()
    def _load_data(self):
        """This function is to load preprocess nuscenes dataset and get the disp gt
        Args:
            dataset_root
        Returns:
            sample_seq_files:list
        """
        if self.dataset_root is None:
            self.dataset_root = '/home/qianka/Motion/WeakMotion/WeakMotion-New-2/data/input-data/trainval'

        scene_dirs = [d for d in os.listdir(self.dataset_root) if os.path.isdir(os.path.join(self.dataset_root, d))]
        ...
        sample_seq_files = []
        for s_dir in scene_dirs:
            sample_dir = os.path.join(self.dataset_root, s_dir)
            sample_files = [os.path.join(sample_dir, f) for f in os.listdir(sample_dir)
                            if os.path.isfile(os.path.join(sample_dir, f))]

            sample_seq_files += sample_files

        self.seq_files = sample_seq_files
    
    @staticmethod
    def gen_2d_grid_gt_general(gt_dict: dict, category_num: int = 5):
        ...
        pixel_cat_map = np.zeros((dims[0], dims[1], category_num), dtype=np.float32)
        ...
        for i in range(num_past_pcs):
            indices = gt_dict[f"voxel_indices_{i}"]
        ...
        return all_disp_field_gt, all_valid_pixel_maps, non_empty_map, pixel_cat_map, pixel_indices


    def _compute_speed_level_general(self, disp_field_gt, total_future_sweeps=20, frame_skip=3):
        """This function is to calculate the speed_level with a certain 
        interval

        Args:
            - disp_field_gt: The disp gt data: [total_future_sweeps, bev_w, bev_h, 2]
            - total_future_sweeps: prediction seq_len
        Returns:
            - motion_status_gt: [bev_w, bev_h, one-hot-interval_num]
        """
        speed_intervals = self.speed_intervals
        
        # selected frames: 4, 8, 12, 16, 20
        selected_future_sweeps = np.arange(0, total_future_sweeps + 1, frame_skip + 1)
        selected_future_sweeps = selected_future_sweeps[1:]
        last_future_sweep_id = selected_future_sweeps[-1]
        distance_intervals = speed_intervals * (last_future_sweep_id / 20.0)

        # speed_level shape: [bev_w, bev_h, num_intervals]
        speed_level = np.zeros(( \
                      disp_field_gt.shape[1], disp_field_gt.shape[2],
                      speed_intervals.shape[0]), dtype=np.float32)
        last_frame_disp_norm = np.linalg.norm(disp_field_gt, ord=2, axis=-1)
    ...
    def _get_speed_level_weight(self, cat_cnt):
        """This function is to calculate the weights for every interval
        Args:
            - intervals(self): interval to split the disp gt [k-interval,2]
            - cat_cnt: the gt num in every interval
        Returns:
            - weights: [k-interval,1]
        """
        speed_interval_width = self._get_interval_width(self.speed_intervals)
        max_node = self.speed_intervals[-1, -1]
        min_node = self.speed_intervals[0, 0]

        ee = 1
        weight = np.array(speed_interval_width) * np.sum(cat_cnt + ee) / ((cat_cnt + ee) * (max_node-min_node))
        # new_weight = np.exp(- 1 / weight)
        new_weight = 1 / (cat_cnt + ee) / np.sum(1 / (cat_cnt + ee))

        return weight, new_weight


    def _load_statistic_info(self):
        """This function is to load the statistic information from .npy
        # Save
        dict = {'a':1,'b':2,'c':3}
        ...
        """
        ret_dict = np.load(os.path.join(self.save_path, \
            'data_statistic_info.npy')).item()

        return ret_dict

    def _save_statistic_info(self, ret_dict:dict):
        """This function is to save the statistic information
        # Save
        dict = {'a':1,'b':2,'c':3}
        np.save('my_file.npy', dict) # 注意带上后缀名
        ...
        np.save(os.path.join(self.save_path, 'data_statistic_info.npy'), ret_dict)
        with open(os.path.join(self.save_path, 'data_statistic_info.pkl'), 'wb') as f:
            pickle.dump(ret_dict, f)
        with open(os.path.join(self.save_path, 'data_statistic_info.json'), 'w') as f_json:
            ret_str = json.dumps(ret_dict)
            f_json.write(ret_str)



def compute_data_general(dataset_root=None, frame_skip=3, voxel_size=(0.25, 0.25, 0.4),
                                split='train', area_extents = np.array([[-32., 32.], [-32., 32.], [-3., 2.]]),
                                num_obj_cat=5,
                                ):

    # class
    interval_creator = BalanceNode(node_num=4, \
        init_node=[0, 0.01, 5, 20, 200],
        node_span=[0, 200])
    speed_intervals = interval_creator._get_interval()
    
    data_statisticor = DataStatistic(dataset_root=dataset_root, save_path='./data',
                                    intervals = speed_intervals)
    
    
    # init mat
    num_motion_cat = speed_intervals.shape[0]
    obj_cat_cnt = np.zeros(num_obj_cat, dtype=np.int64)
    motion_cat_cnt = np.zeros(num_motion_cat, dtype=np.int64)

    for idx, sample_file in enumerate(data_statisticor.seq_files):
        gt_data_handle = np.load(sample_file, allow_pickle=True)
        gt_dict = gt_data_handle.item()
        all_disp_field_gt, all_valid_pixel_maps, non_empty_map, pixel_cat_map_gt, _ \
            = data_statisticor.gen_2d_grid_gt_general(gt_dict = gt_dict)

        # -- Compute speed level ground truth
        # motion_status_gt = compute_speed_level(all_disp_field_gt, frame_skip=frame_skip)
        motion_status_gt = \
            data_statisticor._compute_speed_level_general(all_disp_field_gt, frame_skip=frame_skip)
    ...
    # TODO: save the statistic info and obtain the weight for speed intervals
    weight, inverse_weight = data_statisticor._get_speed_level_weight(cat_cnt=motion_cat_cnt)
    
    ret_dict = dict(
        # statistic_info
        obj_cls_sat = obj_cat_cnt.tolist(),
        motion_cls_sat = motion_cat_cnt.tolist(),
        obj_cls_ratio_sat = obj_cat_ratio.tolist(),
        motion_cls_ratio_sta = motion_cat_ratio.tolist(),
        motion_cls_reverse_ratio_sta = motion_cat_ratio_inverse.tolist(),
        motion_weight = weight.tolist(),
        motion_new_weight = inverse_weight.tolist()
    )
    data_statisticor._save_statistic_info(ret_dict=ret_dict)


if __name__ == "__main__":
    compute_data_general(dataset_root='/home/qianka/Motion/WeakMotion/WeakMotion-New-2/data/input-data/trainval')
