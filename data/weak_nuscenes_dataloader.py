...
import os
import warnings
from data.weak_utils import remove_close, filter_pc, convert_semantic_to_FGBG, gen_voxel_indices_for_pc, get_nongeneral_mask
# if debug
# from weak_utils import remove_close, filter_pc, convert_semantic_to_FGBG, gen_voxel_indices_for_pc, get_nongeneral_mask

class DatasetSingleSeq_Stage1(Dataset):
    """
    Generate the nuScenes training dataset for Stage1
    ...
    def __init__(self, dataset_root=None, split='train', future_frame_skip=0, voxel_size=(0.25, 0.25, 0.4),
        ...
        if split == 'train':
            ...
            for d in os.listdir(self.dataset_root):
                tmp_1 = os.path.join(self.dataset_root, d) + '/0.npy'
                seq_dirs.append(tmp_1)
        ...
        # For training, the size of dataset should be 17065 * 3; for validation: 1719 * 1;
        if split == 'train' and self.num_sample_seqs != 17065 * 3:
            warnings.warn(">> The size of training dataset is not 17065 * 3.\n")
        elif split == 'val' and self.num_sample_seqs != 1719 * 1:
            warnings.warn(">> The size of validation dataset is not 1719 * 3.\n")
    ...
    def __getitem__(self, idx):
        ...
        if self.split == 'train':
            ...
            file_name = os.path.join(os.path.join(self.dataset_root, scene_name), '0.npy')
            ...
            if self.get_general:
            # generate the nongeneral object mask
                non_general_mask = get_nongeneral_mask(label[:, 0]) # 0: non general; 1: general
                FGBG_gt_mask = FGBG_gt_mask * non_general_mask
            ...
            # qka 20240330: convert those == 0 --> 3
            FGBG_gt_mask[FGBG_gt_mask == 0] = 3
            ...
            '''qka 20231211'''
            # print(voxel_indices[:, 0], voxel_indices[:, 1], voxel_indices[:, 2])
            voxel_points = np.zeros(self.dims, dtype=bool)
...
class DatasetSingleSeq_Stage2(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        '''qka 20231212 split name train or trainval'''
        if self.split in ['train', 'trainval']:
            ...
            if self.get_general:
            # generate the nongeneral object mask
                non_general_mask = get_nongeneral_mask(label_seg[:, 0]) # 0: non general; 1: general
                FGBG_gt_mask_seg = FGBG_gt_mask_seg * non_general_mask
            ...
            # qka 20240330: convert those == 0 --> 3
            FGBG_gt_mask_seg[FGBG_gt_mask_seg == 0] = 3
            ...
            if self.annotation_ratio ==1:
                ...
                if self.get_general:
                # generate the nongeneral object mask
                    non_general_mask = get_nongeneral_mask(label_0[:, 0]) # 0: non general; 1: general
                    FGBG_gt_mask_0 = FGBG_gt_mask_0 * non_general_mask
                ...
                if self.get_general:
                # generate the nongeneral object mask
                    non_general_mask = get_nongeneral_mask(label_1[:, 0]) # 0: non general; 1: general
                    FGBG_gt_mask_1 = FGBG_gt_mask_1 * non_general_mask
                ...
                if self.get_general:
                # generate the nongeneral object mask
                    non_general_mask = get_nongeneral_mask(label_2[:, 0]) # 0: non general; 1: general
                    FGBG_gt_mask_2 = FGBG_gt_mask_2 * non_general_mask
...
if __name__ == "__main__":
    ...
    trainset = DatasetSingleSeq_Stage1(dataset_root='./WeakMotion/WeakMotion-New-2/data/weak-data/train/', split='train',
                                           future_frame_skip=0,voxel_size=voxel_size,
                                           area_extents=area_extents, annotation_ratio=1)
    a = trainset[0]