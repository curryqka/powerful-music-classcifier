...
import os
import warnings
from model.model import MotionNet, MotionNetMGDA, FeatEncoder
...
def eval_motion_displacement(model_path=None, save_eval_file_path=None, use_adj_frame_pred=False,
                             ...
                             height_feat_size=13, num_obj_category=5, general_flag = False, which_model='MotionNet'):
    """
    ...
    if save_eval_file_path is not None:
        ...
        logger_path = check_folder(save_eval_file_path)
        logger_file = os.path.join(logger_path, 'eval_displacement.txt')
        saver = open(logger_file, "w")

        # init saver writer
        saver.write(f"The model: {which_model} \n")
        saver.write(f"The general flag: {general_flag} \n")
        saver.write(f"The model path: {model_path} \n")
    ...
    elif which_model == "MotionNetMGDA":  # MotionNetMGDA
    ...
    elif which_model == "En_MotionNet":
        model = En_MotionNet(out_seq_len=20, motion_category_num=2, height_feat_size=height_feat_size)
        model = nn.DataParallel(model)
        checkpoint = torch.load(model_path)
        model.load_state_dict(checkpoint['model_state_dict'])
        model = model.to(device)
    ...
    # Make prediction
    if which_model == 'MotionNet' or 'En_MotionNet':
        model.eval()
    else:
        model_encoder.eval()
        model_head.eval()
...
def eval_motion_displacement_general(model_path=None, save_eval_file_path=None, use_adj_frame_pred=False,
                             dataset_path=None, future_frame_skip=0, split='test',
                             voxel_size=(0.25, 0.25, 0.4), area_extents=np.array([[-30., 30.], [-30., 30.], [-3., 2.]]),
                             num_future_sweeps=20, batch_size=1, num_workers=8, use_motion_state_pred_masking=False,
                             height_feat_size=13, num_obj_category=5, which_model='MotionNet'):
    """
    Evaluate the motion prediction results for general object.
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        # Init category label to nongernal label
        cat_nongeneral_map = np.zeros_like(pixel_cat_map_numpy)

        # Convert the category map to nongerneral map
        cat_nongeneral_map = cat_nongeneral_map[pixel_cat_map_gt == 5] = 1
        # cat_nongeneral_map = (cat_nongeneral_map * non_empty_map_numpy * filter_mask).astype(int)
        ...
        with torch.no_grad():
            ...
            if use_motion_state_pred_masking:
                ...
                cat_nongeneral_map = cat_nongeneral_map[:, np.newaxis, np.newaxis, ...] # (batch, 1, 1, h, w)
                disp_pred = disp_pred * cat_nongeneral_map
...
if __name__ == "__main__":
    ...
    parser.add_argument('-n', '--net', default='MotionNet', type=str, help='Which network [MotionNet/MotionNetMGDA/En_MotionNet]')
    ...
    parser.add_argument('-g','--general', action='store_true', help='Whether to apply general')
