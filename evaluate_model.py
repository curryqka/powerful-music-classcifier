...
parser.add_argument('--model', default='WeakMotionNet', type=str, help='The model to evaluate')

parser.add_argument('--general', action='store_true', help='Whether to choose general')
...
def main():
    ...
    evalloader = torch.utils.data.DataLoader(evalset, batch_size=8, shuffle=False, num_workers=num_workers)
    ...
    model = model_dict[args.model](out_seq_len, 2, height_feat_size)
...
def get_speed_error(cell, saver_, speed_intervals, selected_future_sweeps):
    me_list = np.zeros([3])
    ...
    for i, d in enumerate(speed_intervals):
        ...
        if need_log:
            saver_.write("--------------------------------------------------------------\n")
            saver_.write("For cells within speed range [{}, {}]:\n\n".format(d[0], d[1]))
...
def eval(model, evalloader, device, saver):
    ...
    for i in range(distance_intervals.shape[0]):
        ...
        # cell_groups_general.append(cell_statistics)

    # cell_groups_general = cell_groups.copy()
    ...
    for i, data in tqdm(enumerate(evalloader, 0), total=len(evalloader), smoothing=0.9):
        ...
        with torch.no_grad():
            ...
            if args.model == 'WeakMotionNet':
                ...
                # to cater for the be-sti trainval 【use disp_pred, FGBG_pred, _】
                disp_pred, _, FGBG_pred = model(padded_voxel_points)
            else:
                assert False,"The model only contains [WeakMotionNet/be-sti]"
            ...
            if not get_general:
                ...
                overall_cls_gt, overall_cls_pred, overall_nongeneral_gt, cat_nongeneral_map = evaluate_FGBG_prediction_general(FGBG_pred, non_empty_map_numpy, pixel_cat_map_gt_numpy,
                                                                            overall_cls_gt, overall_cls_pred, overall_nongeneral_gt)
                
                cell_groups = evaluate_motion_prediction_general(disp_pred, FGBG_pred, 
                                                        all_disp_field_gt, pixel_cat_map_gt_numpy, all_valid_pixel_maps, future_steps,
                                                        distance_intervals, selected_future_sweeps, cell_groups)
            
    # print(cell_groups == cell_groups_general)
   
    me_list = get_speed_error(cell_groups, saver, speed_intervals, selected_future_sweeps)
    # me_list_general = get_speed_error(cell_groups_general, saver, speed_intervals, selected_future_sweeps)
