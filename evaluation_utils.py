...
def evaluate_motion_prediction(disp_pred, FGBG_pred, all_disp_field_gt, all_valid_pixel_maps, future_steps,
                               distance_intervals, selected_future_sweeps, cell_groups:list,
...
'''
This function is to get the cat_nongernal_map

'''
def evaluate_FGBG_prediction_general(FGBG_pred, non_empty_map_numpy, pixel_cat_map_gt_numpy, overall_cls_gt, overall_cls_pred, overall_nongernal_map,
                             datatype='nuScenes'):
    ...
    # cat_nongeneral_map = np.zeros_like(pixel_cat_map_numpy, dtype=np.float32)
    cat_nongeneral_map = pixel_cat_map_numpy.copy()

    if datatype == 'nuScenes':
        # 1: background or empty; 2: Vehicle; 3: Ped; 4: Bike; 5: Others
        pixel_FGBG_map_numpy[pixel_FGBG_map_numpy > 1] = 2

        # 1: background or empty; 2: Vehicle; 3: Ped; 4: Bike; 5: 
        tmp = cat_nongeneral_map.copy()
        cat_nongeneral_map[(tmp == 5)] = 1.0
        cat_nongeneral_map[tmp < 5] = 0
        # print(cat_nongeneral_map)
    elif datatype == 'Waymo':
        # 1: background or empty; 2: Vehicle; 3: Ped; 4: Cyclist; 5: Sign, regarded as background
        tmp = pixel_FGBG_map_numpy.copy()
        pixel_FGBG_map_numpy[tmp > 1] = 2
    ...
    pixel_FGBG_map_numpy = (pixel_FGBG_map_numpy * non_empty_map_numpy * filter_mask).astype(
        np.int32)  # 0: Empty; 1: Background; 2: Foreground
    '''
    cat_nongeneral_map = (cat_nongeneral_map * non_empty_map_numpy * filter_mask).astype(
        np.int32)  # 0: Empty , others; 1: other
    '''
    ...
    overall_cls_gt.append(pixel_FGBG_map_numpy[idx_fg])
    overall_cls_pred.append(FGBG_pred_numpy[idx_fg])
    overall_nongernal_map.append(cat_nongeneral_map[idx_fg])
    return overall_cls_gt, overall_cls_pred, overall_nongernal_map, cat_nongeneral_map

'''
This function is to evaluate the motion predition about general object
'''

def evaluate_motion_prediction_general(disp_pred, FGBG_pred, all_disp_field_gt, pixel_cat_map_gt_numpy, 
                               all_valid_pixel_maps, future_steps,
                               distance_intervals, selected_future_sweeps, cell_groups:list,
                               use_FGBG_pred_masking=True, datatype='nuScenes'):
    ...
    # disp_pred = disp_pred * cat_nongeneral_map  # small motion, static, background
    # filter those are nongerneral object
    # nongeneral: 0 general: 1
    ...
    for j, d in enumerate(distance_intervals):
        for slot, s in enumerate((selected_future_sweeps - 1)):  # selected_future_sweeps: [4, 8, ...]
            ...
            # filter those are nongerneral object
            # cat_nongernal_map : cat_weight_map 0:nongeneral 1:general
