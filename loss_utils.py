...
def FGBG_seg_loss(FGBG_pred, point_FGBG_gt_mask, source_pc, source_num, voxel_size, area_extents):
    ...
    for batch_index in range(batch_size):
        ...
        # curr_point_FGBG_gt_map : [curr_source_num, 2]
        ...
        # curr_FGBG_weight_map : [curr_source_num]
        # if we want to mask the 'other' cls, curr_point_other_gt_mask * 0 or just refine the dataloader
        curr_FGBG_weight_map = (curr_point_BG_gt_mask * 0.0005 + curr_point_FG_gt_mask * 2.0).squeeze(0)
        curr_annotated_point_num = torch.sum((curr_point_FGBG_gt_mask != 3).float())

        # filter those not general
        if get_general:
            # Calculate the number of points where curr_point_FGBG_gt_mask is not 3 and not 0
            # curr_annotated_point_num = torch.sum(((curr_point_FGBG_gt_mask != 3) & (curr_point_FGBG_gt_mask != 0)).float())
            pass
        ...
        # curr_point_FGBG_pred: [curr_source_num, 2]
        curr_log_softmax_FGBG_pred = F.log_softmax(curr_point_FGBG_pred, dim=1)
        # - curr_point_FGBG_gt_map * curr_log_softmax_FGBG_pred : [curr_source_num, 2]
