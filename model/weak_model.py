"""
The architecture of PreSegNet and WeakMotionNet
...
class FGBGEstimation(nn.Module):
    def __init__(self, motion_category_num=2):
        super(FGBGEstimation, self).__init__()
...
class STPN(nn.Module):
    def __init__(self, height_feat_size=13):
        ...
        self.padnum = 12
        self.zero_pad = nn.ZeroPad2d(padding=(self.padnum, self.padnum, self.padnum, self.padnum))
    ...
    def forward(self, x):
        ...
        # reserve padding
        # res_x = res_x[:, :, self.padnum:-self.padnum, self.padnum:-self.padnum]
        return res_x


class STPN_Seg(nn.Module):
    def __init__(self, height_feat_size=13):
        super(STPN_Seg, self).__init__()
        '''2023 1225'''
        self.padnum = 12
        self.zero_pad = nn.ZeroPad2d(padding=(self.padnum, self.padnum, self.padnum, self.padnum))
    ...
    def forward(self, x):
        batch, seq, z, h, w = x.size()
        # z : height dim, h,w: x,y dim
        # print(batch, seq, z, h, w )
        x = x.view(-1, x.size(-3), x.size(-2), x.size(-1))
        # padding
        # x = self.zero_pad(x)
        # print(x.size())
