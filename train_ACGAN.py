...
from model.model import MotionNet
from model.en_model import En_MotionNet
from model.en_gan_model import En_GAN_MotionNet
from model.gan_model import GeneratorAC,DiscriminatorAC,DHead,QHead,AHead
from model.en_gan_model import FeatEncoder
# from utlis.model_utlis import NormalNLLLoss
from utlis.model_utlis import *
from data.nuscenes_dataloader import TrainDatasetMultiSeq, DatasetSingleSeq
...
parser.add_argument('--pre_model_path', default='/home/qianka/Motion/WeakMotion/WeakMotion-New-2/pretrained/weak_WeakMotionNet.pth', \
                    help='The path to pretrained backbone model')
parser.add_argument('--model', default="Feat_Encoder", type=str, help="choose model[MotionNet, En_MotionNet, Feat_Encoder]")
...
def main():
    ...
    model_dict = {
        'MotionNet': MotionNet,
        'En_MotionNet': En_MotionNet,
        'Feat_Encoder': FeatEncoder,
    }
    assert args.model in model_dict.keys(),"The model name only support MotionNet and En_MotionNet"
    model = model_dict[args.model](height_feat_size=height_feat_size)
    # model = En_MotionNet(out_seq_len=out_seq_len, motion_category_num=2, height_feat_size=height_feat_size)

    # load the pre-trained WeakMotionNet
    pretrain_model_path = args.pre_model_path
    pre_model = torch.load(pretrain_model_path)
    new_model_dict = model.state_dict()
    # pre_model['model_state_dict'].update({k.split('.')[1:] : v} for k,v in pre_model['model_state_dict'].items())
    for k,v in list(pre_model['model_state_dict'].items()):
        pre_model['model_state_dict']['.'.join(k.split('.')[1:])] =   pre_model['model_state_dict'].pop(k)
    state_dict = {k:v for k, v in pre_model['model_state_dict'].items() if k in new_model_dict.keys() and 'motion' not in k}
    ...
    # TODO: load genertor
    generator = GeneratorAC(input_channel=100)
    generator = nn.DataParallel(generator)
    generator = generator.to(device=device)
    generator = generator.apply(weights_init)

    # TODO: load discriminator
    discriminator = DiscriminatorAC()
    discriminator = nn.DataParallel(discriminator)
    discriminator = discriminator.to(device=device)

    # TODO: load AHead and DHead
    ...
    # TODO: load generator and discriminator optim ACGAN
    """ split the head
    optimD = optim.Adam([{'params': discriminator.parameters()}, {'params': Dnet.parameters()}], lr=1e-4 )
    optimG = optim.Adam([{'params': model_generator.parameters()}, {'params': Anet.parameters()}], lr=1e-4)
    """
    ...
    scheduler_D = torch.optim.lr_scheduler.MultiStepLR(optimD, milestones=[10, 20, 30, 40], gamma=0.5)
    scheduler_G = torch.optim.lr_scheduler.MultiStepLR(optimG, milestones=[10, 20, 30, 40], gamma=0.5)

    # optimInfo = torch.optim.Adam(itertools.chain(generator.parameters(), discriminator.parameters()), lr=0.0016)
    # scheduler_Info = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[10, 20, 30, 40], gamma=0.5)
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        loss_disp, loss_class, loss_motion, loss_bg_tc, loss_sc, loss_fg_tc \
            = train(model, discriminator, generator, criterion, trainloader, optimizer, optimD, optimG, device, epoch)
        ...
        me_static, me_slow, me_fast, cat_map, mean_cat, pixel_acc = \
            eval_motion_displacement(model=[model, discriminator], dataloader=evalloader, dataset=evalset, device=device)
        ...
        save_dict = {'epoch': epoch,
                    ...
                    'head_state_dict': discriminator.state_dict(),
                    'optimizer_state_dict': optimizer.state_dict(),
                    'optimizerD_state_dict': optimD.state_dict(),
                    'scheduler_state_dict': scheduler_D.state_dict(),
                    'scheduler_state_dict': scheduler.state_dict(),
                    'loss': loss_disp.avg}
        torch.save(save_dict, os.path.join(model_save_path, 'last.pth'))
...
def train(model, D, G, criterion, trainloader, optimizer, optimD, optimG, device, epoch):
    ...
    running_loss_D = AverageMeter('Discriminator',':.6f')
    for i, data in enumerate(trainloader, 0):
        ...
        gt = gt.reshape(gt.shape[0], -1, gt.shape[2], gt.shape[3])

        pixel_instance_map_label = pixel_instance_map.view(-1, 256, 256)
        pixel_instance_map_label = pixel_instance_map_label.unsqueeze(1)
        ...
        pixel_cat_map_label = pixel_cat_map.permute(0, 3, 1, 2)

        all_valid_pixel_maps_label = all_valid_pixel_maps.view(-1, all_valid_pixel_maps.shape[2], 256, 256)
        fake_target = torch.concat([gt, all_valid_pixel_maps_label, \
        ...
        loss_fake, lossD_fake, loss_disp, loss_class, loss_motion, loss_bg_tc, loss_sc, loss_fg_tc = \
        ...
        # Forward prediction and discriminator true
        shared_feats = model(padded_voxel_points)
        disp_pred, class_pred, motion_pred, true_pred = D(shared_feats)

        # Compute and back-propagate the losses
        loss_true, lossD_true, loss_disp, loss_class, loss_motion, loss_bg_tc, loss_sc, loss_fg_tc = \
            compute_and_bp_loss(optimizer, device, num_future_frames[0].item(), all_disp_field_gt, all_valid_pixel_maps,
                                pixel_cat_map_gt, disp_pred, criterion, non_empty_map, class_pred, motion_gt,
                                motion_pred, trans_matrices, pixel_instance_map, true_pred)
        loss = loss_true + 0.005 * loss_fake
        
        # Calculate W-div gradient penalty
        gradient_penalty = calculate_gradient_penalty(D,
                                                        shared_feats, fake_feats,
                                                        device)
        
        lossD = (-lossD_true + lossD_fake + gradient_penalty * 10)*0.05
        ...
        if train_G:
            if (i + 1) % 100 == 0:
                ...
                compute_and_bp_loss(optimizer, device, num_future_frames[0].item(), all_disp_field_gt, all_valid_pixel_maps,
                                    pixel_cat_map_gt, fake_disp_pred, criterion, non_empty_map, fake_class_pred, motion_gt,
                                    fake_motion_pred, trans_matrices, pixel_instance_map, fake_pred)
                lossD = -lossD_fake*0.05
                loss = 0.0005 * loss_fake
        ...
        if i % 10 == 0:
            print("[{}/{}]\t{}, \t{}, \t{}, \t{}, \t{}, \t{}, \t{}".
                format(epoch, i, running_loss_disp, running_loss_class, running_loss_motion, running_loss_bg_tc,
                     running_loss_sc, running_loss_fg_tc, lossD))
...
def eval_motion_displacement(model, dataloader, dataset, device, save_eval_file_path=None, use_adj_frame_pred=True,
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        with torch.no_grad():
            ...
            feats = model[0](padded_voxel_points)
            disp_pred, class_pred, motion_pred, _ = model[1](feats)
...
def compute_and_bp_loss(optimizer, device, future_frames_num, all_disp_field_gt, all_valid_pixel_maps, pixel_cat_map_gt,
    ...
    return loss, lossD, loss_disp.item(), loss_class.item(), loss_motion.item(), bg_tc_loss_value, \
