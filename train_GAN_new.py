...
from model.gan_model import Generator,Discriminator,DHead,QHead
...
parser.add_argument('--model', default="FeatEncoder", type=str, help="choose model[MotionNet, En_MotionNet, FeatEncoder]")
...
def main():
    ...
    model = model_dict[args.model](out_seq_len=out_seq_len, motion_category_num=2, height_feat_size=height_feat_size)
    ...
    # TODO: load genertor
    generator = Generator(input_channel=200)
    ...
    Qnet, Dnet = QHead(), DHead()
    Qnet, Dnet = nn.DataParallel(Qnet), nn.DataParallel(Dnet)
    Qnet, Dnet = Qnet.to(device=device), Dnet.to(device=device)
    ...
    optimG = optim.Adam([{'params': model_generator.parameters()}, {'params': Qnet.parameters()}], lr=1e-4)
    """
    # Loss for continuous latent code.
    criterionQ_L = NormalNLLLoss()
    optimD = optim.Adam(discriminator.parameters(), lr=1e-4)
    optimG = optim.Adam(generator.parameters(), lr=1e-4)
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        me_static, me_slow, me_fast, cat_map, mean_cat, pixel_acc = \
            eval_motion_displacement(model=model, dataloader=evalloader, dataset=evalset, device=device)
...
def train(model, D, G, criterion, criterionQ_L, trainloader, optimizer, optimD, optimG, optimInfo, device, epoch):
    ...
    for i, data in enumerate(trainloader, 0):
        ...
        disp_pred, class_pred, motion_pred, true_prob, mu, var = D(shared_feats)
...
def compute_and_bp_loss(optimizer, device, future_frames_num, all_disp_field_gt, all_valid_pixel_maps, pixel_cat_map_gt,
    ...
    if use_weighted_loss:  # Note: have also tried focal loss, but did not observe noticeable improvement
        ...
        disp_weight_map = disp_weight_map.reshape(map_shape[0], -1, map_shape[-3], map_shape[-2], map_shape[-1])
        disp_weight_map = torch.sum(disp_weight_map, dim=1).unsqueeze(1)
        disp_weight_map = F.normalize(disp_weight_map, dim=1, p=2)

        loss_disp = criterion(gt * valid_pixel_maps, disp_pred * valid_pixel_maps)
        loss_disp = loss_disp.reshape(map_shape[0], -1, map_shape[-3], map_shape[-2], map_shape[-1])
        loss_disp = torch.sum(loss_disp * disp_weight_map) / valid_pixel_num
        '''
    else:
