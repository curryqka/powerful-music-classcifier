...
def main():
    ...
    voxel_size = (0.3, 0.3, 0.4)
    if datatype == 'nuScenes':
        # area_extents = np.array([[-32., 32.], [-32., 32.], [-3., 2.]])
        area_extents = np.array([[-54., 54.], [-54., 54.], [-3., 5.]])
    elif datatype == 'Waymo':
    ...
    trainset_split = tmp.split('/')[-1] if tmp.split('/')[-1] != '' else tmp.split('/')[-2]
    print("The split is {}".format(trainset_split))
    if datatype == 'nuScenes':
    ...
    evalset_split = tmp.split('/')[-1] if tmp.split('/')[-1] != '' else tmp.split('/')[-2]
    if datatype == 'nuScenes':
    ...
    if args.resume != '':
        '''qka 20240105 add model file'''
        model_file = os.path.join(args.resume, 'epoch_58_0.043_0.421_1.665_0.957_0.935.pth')
